...
if not found_bands:
    raise ValueError("[ERROR] No band-power columns found in the CSV.")
...
for i in range(n_win):
    ...
    # ── Band power: Convert Muse Bels to Linear Power
    row = {}
    bp_linear = {}   # band → array of per-channel linear values
    for band in BANDS:
        ch_vals = []
        for ch, col in band_cols[band].items():
            vals = pd.to_numeric(win[col], errors='coerce').dropna()
            if len(vals) > 0:
                # Muse gives values in Bels (log10). Convert to linear power: 10^value
                linear_vals = 10 ** vals 
                ch_vals.append(float(linear_vals.mean()))
        bp_linear[band] = np.array(ch_vals) if ch_vals else np.array([0.0])

    # Calculate total power to convert everything to percentages (0.0 to 1.0)
    ...
    # Group B: std relative power per band (FIXED: loops through all 5 bands now)
    for band in BANDS:
    ...
    # Group D: spatial / regional features (FIXED to use linear relative values)
    def get_ch_rel_power(band, ch):
        if ch in band_cols[band]:
            vals = pd.to_numeric(win[band_cols[band][ch]], errors='coerce').dropna()
            if len(vals) > 0:
                return float((10 ** vals).mean()) / total_power
        return np.nan

    # frontal_alpha_asymm → AF7_Alpha − AF8_Alpha
    af7_a = get_ch_rel_power('Alpha', 'AF7')
    af8_a = get_ch_rel_power('Alpha', 'AF8')
    row['frontal_alpha_asymm'] = (af7_a - af8_a) if not np.isnan(af7_a) and not np.isnan(af8_a) else 0.0

    # frontal_theta_mean → mean(AF7_Theta, AF8_Theta)
    ft_vals = [get_ch_rel_power('Theta', ch) for ch in ['AF7', 'AF8']]
    ft_vals = [v for v in ft_vals if not np.isnan(v)]
    row['frontal_theta_mean'] = float(np.mean(ft_vals)) if ft_vals else row['mean_rel_Theta']

    # occipital_alpha_mean → mean(TP9_Alpha, TP10_Alpha)
    oa_vals = [get_ch_rel_power('Alpha', ch) for ch in ['TP9', 'TP10']]
    oa_vals = [v for v in oa_vals if not np.isnan(v)]
